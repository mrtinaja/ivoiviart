...
const getFrontendBase = (req) => {
    ...
    if (isHttpUrl(fromOrigin))
        return trimSlash(fromOrigin);
    return "http://localhost:3000";
};
const idemKey = (obj) => crypto.createHash("sha256").update(JSON.stringify(obj)).digest("hex");
/* -------------- Endpoint ------------- */
const createPreference = async (req, res) => {
    ...
    const preference = { items, back_urls };
